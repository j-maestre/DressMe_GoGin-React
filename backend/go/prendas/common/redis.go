...
func SaveUserLike(user uint, prenda string, client *redis.Client) error{

	var newUsers string;

	//Pillamos el total de likes que tiene esa prenda
	totalUsers, err2 := client.Get(ctx, prenda).Result()
	...
	err3 := client.Set(ctx,prenda, newUsers, 0).Err()
